# Project & Owner Configuration Worksheet

Fill in these identifiers as they are generated during Google Play Console and Google Cloud setup.

> [!CAUTION]
> Do NOT store private passwords, signing keystores, or OAuth client secrets in this file or any version-controlled file.

---

```yaml
Play app name: War of Attrition - Card Game
Android package ID: com.cboler.warofattrition
GitHub Pages URL: https://cboler.github.io/war-of-attrition-game/
AssetLinks Origin: https://cboler.github.io/.well-known/assetlinks.json

Google Cloud Project ID: war-of-attrition-game
Google Cloud Project Number: 334487063631

Web OAuth Client ID: 334487063631-650tq3qf40o5286k609pbrn39c2i4d9a.apps.googleusercontent.com
Android OAuth Client ID: 334487063631-6ta1hfao1fs3qjnqten573kvubpib21e.apps.googleusercontent.com
Play Games Services numeric Project ID: 334487063631

Upload Keystore SHA-1: D2:EF:37:FC:26:52:EB:60:B8:B0:19:5D:26:36:DF:08:53:1F:60:7F
Upload Keystore SHA-256: E6:4C:7F:6D:AE:84:DE:74:CE:A4:07:B6:32:99:92:55:88:82:F9:A8:85:CA:7A:2D:32:E0:0E:A2:8F:5A:F4:BE

Play App Signing Certificate SHA-1: <PASTE_PLAY_APP_SIGNING_SHA1>
Play App Signing Certificate SHA-256: <PASTE_PLAY_APP_SIGNING_SHA256>

Support Email: requiredcheese@gmail.com
Privacy URL: https://cboler.github.io/war-of-attrition-game/privacy/
Support URL: https://cboler.github.io/war-of-attrition-game/support/
Data Deletion URL: https://cboler.github.io/war-of-attrition-game/delete-account/
Internal Testing Join URL: <PASTE_PLAY_INTERNAL_TEST_OPT_IN_LINK>
```
