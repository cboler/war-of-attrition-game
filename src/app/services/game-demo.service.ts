  ...
  runGameDemo(): string[] {
    ...
    for (let i = 1; i <= 3; i++) {
      ...
      while (settledResult.nextPhase === GamePhase.BATTLE) {
        ...
        if (settledResult.pendingBattleSettlement && settledResult.battleOutcome) {
          this.turnResolutionService.finalizeBattle(
            settledResult.battleOutcome,
      ...
      log.push(
        `   Current cards - Player: ${this.gameStateService.currentStats.playerCardCount}, Opponent: ${this.gameStateService.currentStats.opponentCardCount}`,
      );
      log.push('');
