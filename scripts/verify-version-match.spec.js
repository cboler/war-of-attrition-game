...
test('normalizes supported workflow version-name forms', () => {
  assert.equal(normalizeVersionName('v4.2.9', 'version'), '4.2.9');
...
test('accepts only an Android version name matching the package version', () => {
  assert.equal(assertVersionNameMatchesPackage('v4.2.9', '4.2.9'), '4.2.9');
  assert.throws(
    () => assertVersionNameMatchesPackage('4.2.8', '4.2.9'),
    /does not match the authoritative web\/package version/,
  );
});

test('keeps the checked-in authoritative package version at 4.2.9', () => {
  assert.equal(readPackageVersion(), '4.2.9');
});
